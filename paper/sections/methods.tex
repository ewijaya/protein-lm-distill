...
To assess whether distilled models serve as effective starting points for domain
adaptation, we fine-tuned all five models on two protein families: conotoxin
(PF02950, median 68 AA) and lysozyme (PF00959, median 170 AA), at five training
subset sizes ($N \in \{50, 100, 200, 500, 1000\}$). All models were trained with
AdamW, cosine learning rate schedule, early stopping (patience~3), effective batch
size~8, and FP16 mixed precision; learning rates were scaled by model size
(Table~\ref{tab:ft_hyperparams}). For each fine-tuned model, we generated 200
sequences and evaluated them with HMMER~\cite{eddy2011hmmer} against family Pfam HMM
profiles ($E < 10^{-5}$). The teacher required gradient checkpointing on a 24\,GB L4
GPU; students did not. Full hyperparameter, dataset, and generation configuration
details are provided in Tables~\ref{tab:ft_hyperparams}--\ref{tab:ft_generation}.
